...
class TransformerEncoderLayer(nn.Module):
    ...
    def with_pos_embed(tensor, pos_embed):
        ...
        if tensor.shape[1] != pos_embed.shape[1]:
            # If sizes don't match, return tensor without pos_embed (fallback)
            # This can happen during profiling with different input sizes
            return tensor
    ...
    def forward(self, feats):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                # Always build positional embeddings dynamically in training mode
                # or when eval_spatial_size is None, or when cached embeddings don't match
                if self.training or self.eval_spatial_size is None:
                    pos_embed = self.build_2d_sincos_position_embedding(
                        w, h, self.hidden_dim, self.pe_temperature).to(src_flatten.device)
                else:
                    # Try to use cached embeddings, but verify size matches
                    ...
                    if cached_pos_embed is not None:
                        ...
                        # Check if cached embedding size matches actual feature size
                        if cached_pos_embed.shape[1] == src_flatten.shape[1]:
                            pos_embed = cached_pos_embed
                        else:
                            # Size mismatch, build dynamically
                            pos_embed = self.build_2d_sincos_position_embedding(
                                w, h, self.hidden_dim, self.pe_temperature).to(src_flatten.device)
                    else:
                        # No cached embedding, build dynamically
                        pos_embed = self.build_2d_sincos_position_embedding(
                            w, h, self.hidden_dim, self.pe_temperature).to(src_flatten.device)
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            # Use target size instead of scale_factor to ensure exact size matching
            _, _, h_low, w_low = feat_low.shape
            upsample_feat = F.interpolate(feat_heigh, size=(h_low, w_low), mode='nearest')
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            _, _, h_target, w_target = feat_height.shape
            _, _, h_down, w_down = downsample_feat.shape
            if h_down != h_target or w_down != w_target:
                # Resize downsample_feat to match feat_height size
                downsample_feat = F.interpolate(downsample_feat, size=(h_target, w_target), mode='nearest')
