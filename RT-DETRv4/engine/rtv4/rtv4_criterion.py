...
class RTv4Criterion(nn.Module):
    ...
    def __init__(self, \
        ...
        self._distill_resize_warned = False  # Flag to only warn once about resizing
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if (H_s, W_s) != (H_t, W_t):
            if not self._distill_resize_warned:
                _logger.warning(
                    f"[RTv4Criterion] Resizing teacher feature map from {H_t}x{W_t} to student's {H_s}x{W_s} for distillation. (This message will only appear once)")
                self._distill_resize_warned = True
    ...
    def loss_text_align(self, outputs, targets, indices, num_boxes, **kwargs):
        pred_embeds = outputs.get('pred_embeds')
        text_embeddings = outputs.get('text_embeddings')
        if pred_embeds is None or text_embeddings is None:
            device = next(iter(outputs.values())).device
            return {'loss_text': torch.tensor(0.0, device=device)}
        ...
        src_embeds = pred_embeds[idx]
        text_embeddings = text_embeddings.to(device=src_embeds.device, dtype=src_embeds.dtype)
        target_text = text_embeddings[target_classes_o]

        src_norm = F.normalize(src_embeds, dim=-1)
        tgt_norm = F.normalize(target_text, dim=-1)
        loss = 1 - (src_norm * tgt_norm).sum(-1)
        return {'loss_text': loss.mean()}
