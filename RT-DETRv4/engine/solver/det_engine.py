...
def train_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        text_prompts = targets[0].get('text_prompts') if targets else None
        text_embeddings = targets[0].get('text_embeddings') if targets else None
        if text_prompts is None and hasattr(data_loader.dataset, 'text_prompts'):
            text_prompts = data_loader.dataset.text_prompts
...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator, device):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        outputs = model(samples, text_prompts=text_prompts, text_embeddings=text_embeddings)
